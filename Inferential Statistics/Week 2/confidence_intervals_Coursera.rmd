...
```{r set-seed}
set.seed(97564)                 # make sure to change the seed
```
...
summarise(samp)
ggplot(data = samp, aes(x=area)) + geom_histogram(binwidth = 700)
```
...
<ol>
<li> <span style="color:red">True.</span> </li>
<li> False. </li>
</ol>
...
<li> <span style="color:red">The sample distribution must be nearly normal.</span> </li>
</ol>
...
<li> <span style="color:red">95% of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames, Iowa.</span> </li>
...
# type your code for the Exercise here, and Knit
lower = mean(samp$area) - z_star_95 * (sd(samp$area) / sqrt(nrow(samp)))
upper = mean(samp$area) + z_star_95 * (sd(samp$area) / sqrt(nrow(samp)))
pop_mu = mean(ames$area)

print(c("The upper limit of the 95% confidence interval:",upper))
print(c("The lower limit of the 95% confidence interval:",lower))
print(c("The population mean:",pop_mu))

if(pop_mu >= lower && pop_mu <= upper){
  print("The population mean lies within the 95% confidence interval")
}else{
  print("The population mean does not lie within the 95% confidence interval")
}
```
...
<li> 1% </li>
<li> 5% </li>
<li> <span style="color:red">95%</span> </li>
<li> 99% </li>
</ol>
...
<li> 1.96 </li>
<li> 2.33 </li>
<li> <span style="color:red">2.58</span> </li>
</ol>
...
ci_99 <- ames %>%
        rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
        summarise(lower = mean(area) - z_cv_99 * (sd(area) / sqrt(n)),
                  upper = mean(area) + z_cv_99 * (sd(area) / sqrt(n)))
