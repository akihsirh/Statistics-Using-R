...
<li> <span style="color:red">50\% of houses in Ames are smaller than 1,499.69 square feet.</span> </li>
...
<li> <span style="color:red"> Sample size of 1000. </span></li>
</ol>
...
<span style="color:red"> There are 15000 elements </span>
...
<li> 0 </li>
<li> 3 </li>
<li> <span style="color:red">25 </span> </li>
<li> 100 </li>
<li> 5,000 </li>
</ol>
...
count_of_small_sample
```

4. Which of the following is **true** about the elements in the sampling distributions you created?
<ol>
<li> <span style="color:red">Each element represents a mean square footage from a simple random sample of 10 houses.</span> </li>
...
<ol>
<li><span style="color:red"> decreases </span></li>
<li> increases </li>
...
<span style="color:red"> The best point estimate at this point for the population mean for price is the sample mean </span>
...
sample_means50_price %>% summarise(samp_50_mean = mean(x_bar_price), samp_50_med =median(x_bar_price),
            samp_50_stddev = sd(x_bar_price), samp_50_min = min(x_bar_price), 
            samp_50_max = max(x_bar_price),
            samp_50_q1 = quantile(x_bar_price, 0.25),  # first quartile, 25th percentile
            samp_50_q3 = quantile(x_bar_price, 0.75) # third quartile, 75th percentile
            ) 

```

<span style="color:red"> The mean home price of the population mean for price is 180803.3 </span>
...
sample_means150_price <- ames %>% rep_sample_n(size = 150, reps = 5000, replace = TRUE) %>% 
  summarise(x_bar_price = mean(price))

ggplot(data = sample_means150_price, aes(x= x_bar_price)) + geom_histogram(binwidth = 2000)

sample_means150_price %>% summarise(samp_50_mean = mean(x_bar_price), samp_50_med =median(x_bar_price),
            samp_50_stddev = sd(x_bar_price), samp_50_min = min(x_bar_price), 
            samp_50_max = max(x_bar_price),
            samp_50_q1 = quantile(x_bar_price, 0.25),  # first quartile, 25th percentile
            samp_50_q3 = quantile(x_bar_price, 0.75) # third quartile, 75th percentile
...
<span style="color:red"> Based on the above sample the mean sale price of homes in Ames is 180726.7 </span>
...
# type your code for this Exercise here, and Run Document
sample_15_price <- ames %>% sample_n(size = 15) %>% 
  summarise(x_bar_price = mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15_price <- ames %>% rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>% 
  summarise(x_bar_price = mean(price))

ggplot(data = sample_means15_price, aes(x= x_bar_price)) + geom_histogram(binwidth = 4000)

sample_means15_price %>% summarise(samp_15_mean = mean(x_bar_price), samp_15_med =median(x_bar_price),
            samp_15_stddev = sd(x_bar_price), samp_15_min = min(x_bar_price), 
            samp_15_max = max(x_bar_price),
            samp_15_q1 = quantile(x_bar_price, 0.25),  # first quartile, 25th percentile
            samp_15_q3 = quantile(x_bar_price, 0.75) # third quartile, 75th percentile
            ) 
...
# type your code for this Exercise here, and Run Document
sample_means150_price <- ames %>% rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>% 
  summarise(x_bar_price = mean(price))

ggplot(data = sample_means150_price, aes(x= x_bar_price)) + geom_histogram(binwidth = 2000)

sample_means150_price %>% summarise(samp_150_mean = mean(x_bar_price), samp_150_med =median(x_bar_price),
            samp_150_stddev = sd(x_bar_price), samp_150_min = min(x_bar_price), 
            samp_150_max = max(x_bar_price),
            samp_150_q1 = quantile(x_bar_price, 0.25),  # first quartile, 25th percentile
            samp_150_q3 = quantile(x_bar_price, 0.75) # third quartile, 75th percentile
            ) 
```
...
<li> <span style="color:red"> The means for the two sampling distribtuions are roughly similar. </span> </li> 
